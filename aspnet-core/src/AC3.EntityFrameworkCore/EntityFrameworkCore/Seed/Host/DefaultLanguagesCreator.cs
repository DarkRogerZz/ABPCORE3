...
namespace AC3.EntityFrameworkCore.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly AC3DbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = AC3Consts.MultiTenancyEnabled ? null : (int?)MultiTenancyConsts.DefaultTenantId;
        ...
        public DefaultLanguagesCreator(AC3DbContext context)
        {
            _context = context;
        }
